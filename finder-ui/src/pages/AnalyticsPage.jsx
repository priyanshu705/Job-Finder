...
export default function AnalyticsPage() {
  ...
  const lineData  = trends.map(r => ({ date: r.day?.slice(5), applied: r.applied || 0, failed: r.failed || 0, uncertain: r.uncertain || 0 }))
  const rateData  = trends.map(r => {
    const total = (r.applied || 0) + (r.failed || 0)
    return { date: r.day?.slice(5), rate: total ? Math.round((r.applied / total) * 100) : 0 }
  })
  ...
  const totalFailed   = overview.rejected || 0
  ...
  return (
    <div className="space-y-6" style={{ animation: 'fadeIn 0.4s ease-out both' }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard label="Interviews"     value={overview.interviews || 0} color="slate" sub="all time" loading={loading} />
      </div>
